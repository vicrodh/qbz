...
impl DlnaConnection {
    ...
    pub async fn set_volume(&mut self, volume: f32) -> Result<(), DlnaError> {
        ...
        // Convert through the renderer's declared range, then dedupe on the
        // DEVICE value: two fractions that round to the same step are the same
        // write, and on a 0..31 renderer many do.
