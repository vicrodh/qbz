    ...
    fn draw<R: GlyphRenderer>(
        ...
        let mut lines = self
            ...
            .take_while(move |line| {
                // The FIRST line always draws, even when its metrics overflow
                // max_physical_height: culling it would elide the text to
                // literally NOTHING. CJK fallback fonts report line metrics
                // taller than a snug fixed-height box (Hiragino at 14px is
                // ~21px tall inside TrackRow's 20px title line on macOS), so
                // every CJK track title vanished while Latin ones fit (#543).
                let first = std::mem::take(&mut first_line);
                let metrics = line.metrics();
                first || match layout.max_physical_height {
