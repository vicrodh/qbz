<script lang="ts">
  ...
  import {
    initBlacklistStore,
    isBlacklisted as isArtistBlacklisted,
  ...
  import {
    closeContentSidebar,
    subscribeContentSidebar,
    type ContentSidebarType,
  } from "$lib/stores/sidebarStore";
  ...
  let userPlaylists = $state<
    { id: number; name: string; tracks_count: number }[]
  >([]);
  ...
  let lyricsStatus = $state<
    "idle" | "loading" | "loaded" | "error" | "not_found"
  >("idle");
  ...
  async function loadFavoritesDefaultTab(): Promise<void> {
    try {
      const prefs = await invoke<FavoritesPreferences>(
  ...
  async function handleAlbumClick(albumId: string) {
    try {
      ...
      console.warn(
        "[DEBUG-43] Album API response: id=",
        album?.id,
        "title=",
        album?.title,
        "tracksCount=",
      ...
      console.warn(
        "[DEBUG-43] Converted album: id=",
        converted?.id,
        "tracksCount=",
        converted?.tracks?.length ?? 0,
      );

      if (!converted || !converted.id) {
        console.error(
          "[DEBUG-43] convertQobuzAlbum returned invalid data:",
          converted,
        );
  ...
  async function fetchAlbumArtistAlbums(artistId: number) {
    try {
      const artist = await invoke<QobuzArtist>("get_artist_detail", {
        artistId,
      });
      ...
      console.error(
        'Failed to fetch artist albums for "By the same artist":',
        err,
      );
      albumArtistAlbums = [];
    }
  ...
  async function handleMusicianClick(name: string, role: string) {
    ...
    try {
      const musician = await invoke<ResolvedMusician>("resolve_musician", {
        name,
        role,
      });
  ...
  async function handleContextNavigation() {
    ...
    const requestFocus = (
      contextType: typeof context.type,
      contextId: string,
    ) => {
  ...
  async function playAlbumById(albumId: string) {
    ...
    const quality =
      firstTrack.hires && firstTrack.bitDepth && firstTrack.samplingRate
  ...
  async function queueAlbumNextById(albumId: string) {
    ...
    showToast(
      $t("toast.playingTracksNext", { values: { count: album.tracks.length } }),
      "success",
    );
  }
  ...
  async function queueAlbumLaterById(albumId: string) {
    ...
    if (success) {
      showToast(
        $t("toast.addedTracksToQueue", {
          values: { count: queueTracks.length },
        }),
        "success",
      );
    } else {
  ...
  async function shareAlbumSonglinkById(albumId: string) {
    try {
      ...
      const response = await invoke<{ pageUrl: string }>(
        "share_album_songlink",
        {
          upc: album.upc || null,
          albumId: album.id,
          title: album.title,
      ...
      showToast(
        $t("toast.albumLinkError", { values: { error: String(err) } }),
        "error",
      );
    }
  }
  ...
  async function downloadAlbumById(albumId: string) {
    ...
    showToast(
      $t("toast.preparingTracksOffline", {
        values: { count: tracksToDownload.length, album: album.title },
      }),
      "info",
    );
  ...
  async function fetchPlaylistData(
    ...
    try {
      const playlist = await invoke<PlaylistData>("get_playlist", {
        playlistId,
      });
      return playlist;
    } catch (err) {
  ...
  async function playPlaylistById(playlistId: number) {
    ...
    const artwork =
      firstTrack.album?.image?.large ||
      firstTrack.album?.image?.thumbnail ||
      firstTrack.album?.image?.small ||
      "";
    const quality =
      firstTrack.hires_streamable &&
      firstTrack.maximum_bit_depth &&
      firstTrack.maximum_sampling_rate
  ...
  async function queuePlaylistNextById(playlistId: number) {
    ...
    showToast(
      $t("toast.playingTracksNext", { values: { count: tracks.length } }),
      "success",
    );
  }
  ...
  async function copyPlaylistToLibraryById(playlistId: number) {
    try {
      ...
      showToast(
        $t("toast.failedCopyPlaylist", { values: { error: String(err) } }),
        "error",
      );
    }
  }
  ...
  async function removePlaylistFavoriteById(playlistId: number) {
    try {
      ...
      showToast(
        $t("toast.failedRemoveFavorites", { values: { error: String(err) } }),
        "error",
      );
    }
  }
  ...
  async function handleAlbumTrackPlay(track: Track) {
    ...
    if (selectedAlbum?.tracks) {
      const trackIndex = selectedAlbum.tracks.findIndex(
        (t) => t.id === track.id,
      );
      const trackIds = selectedAlbum.tracks.map((t) => t.id);

      console.log(
        "[Album] Creating context with",
        trackIds.length,
        "tracks, starting at",
        trackIndex,
      );
    ...
    const quality =
      track.hires && track.bitDepth && track.samplingRate
    ...
    if (selectedAlbum?.tracks) {
      console.log(
        "[Album Queue] Building queue from",
      ...
      console.log(
        "[Album Queue] Mapped to",
        queueTracks.length,
        "queue tracks (filtered), startIndex:",
        trackIndex,
      );
      console.log(
        "[Album Queue] Track IDs:",
  ...
  async function toggleShuffle() {
    ...
    if (result.success) {
      showToast(
        result.enabled
          ? $t("toast.shuffleEnabled")
          : $t("toast.shuffleDisabled"),
        "info",
      );
  ...
  async function isTrackAvailable(track: BackendQueueTrack): Promise<boolean> {
    ...
    try {
      const localIds = await invoke<number[]>(
    ...
    await playTrack(
      {
        ...
        samplingRate:
          isLocal && track.sample_rate
            ? track.sample_rate / 1000
            : (track.sample_rate ?? undefined),
        isLocal,
  ...
  async function handleQueueTrackPlay(trackId: string) {
    try {
      ...
      const allTracks = [
        ...
        ...queueState.upcoming,
      ].filter(Boolean) as BackendQueueTrack[];
  ...
  function handleToggleInfinitePlay() {
    ...
    try {
      localStorage.setItem(
        "qbz-infinite-play",
        JSON.stringify(infinitePlayEnabled),
      );
    } catch {
    ...
    showToast(
      infinitePlayEnabled
        ? $t("toast.infinitePlayEnabled")
        : $t("toast.infinitePlayDisabled"),
      "info",
    );
  }
  ...
  async function handleShuffleAlbum() {
    ...
    console.log(
      "[Album Shuffle] Starting shuffle with",
    ...
    console.log(
      "[Album Shuffle] Starting from random track index:",
      randomIndex,
      "track:",
      randomTrack.title,
    );
  ...
  async function handleAddAlbumToQueueNext() {
    ...
    showToast(
      $t("toast.playingTracksNext", {
        values: { count: playableTracks.length },
      }),
      "success",
    );
  }
  ...
  async function handleTrackDownload(track: Track) {
    try {
      ...
      showToast(
        $t("toast.preparingTrackOffline", { values: { title: track.title } }),
        "info",
      );
    } catch (err) {
  ...
  async function handleTrackReDownload(track: Track | DisplayTrack) {
    try {
      ...
      await cacheTrackForOffline({
        ...
        durationSecs:
          "durationSeconds" in track ? track.durationSeconds : track.duration,
      ...
      showToast(
        $t("toast.refreshingTrackOffline", { values: { title: track.title } }),
        "info",
      );
    } catch (err) {
  ...
  async function handleDownloadAlbum() {
    ...
    showToast(
      $t("toast.preparingTracksOffline", {
        values: { count: tracksToDownload.length, album: selectedAlbum.title },
      }),
      "info",
    );
  ...
  async function handleReDownloadAlbum() {
    ...
    showToast(
      $t("toast.refreshingAlbumOffline", {
        values: { album: selectedAlbum.title },
      }),
      "info",
    );
  ...
  async function reDownloadAlbumById(albumId: string) {
    try {
      ...
      showToast(
        $t("toast.refreshingAlbumOffline", { values: { album: album.title } }),
        "info",
      );
  ...
  async function handleDisplayTrackDownload(track: PlaylistTrack) {
    try {
      const quality =
        track.bitDepth && track.samplingRate
          ? `${track.bitDepth}bit/${track.samplingRate}kHz`
          : track.hires
            ? "Hi-Res"
            : "-";
  ...
  async function createContextAndPlayDisplayTrack(
    ...
    console.log(
      `[Context] Created for ${contextType}: ${contextLabel}, starting at index ${startIndex}`,
    );
  ...
  async function handleLoginSuccess(info: UserInfo) {
    setLoggedIn(info);
    showToast(
      $t("toast.welcomeUser", { values: { name: info.userName } }),
      "success",
    );
  ...
  async function checkAlbumFullyDownloaded(albumId: string): Promise<boolean> {
    ...
    try {
      const isDownloaded = await invoke<boolean>("check_album_fully_cached", {
        albumId,
      });
  ...
  onMount(() => {
    ...
    registerAction("ui.showShortcuts", () => {
          ...
          if (position > 5) {
            setTimeout(async () => {
              try {
                ...
                console.log(
                  "[CastHandoff] Could not restore position:",
                  seekErr,
                );
              }
            }, 1000);
    ...
    const unsubscribeContentSidebar = subscribeContentSidebar(
      (active: ContentSidebarType) => {
        if (active === "network") {
          hideLyricsSidebar();
          closeQueue();
        }
    ...
    setOnResumeFromStop(async () => {
      ...
      if (queueState?.current_track && queueState.current_index !== null) {
        console.log(
          "[Player] Resuming from stop, replaying queue index:",
          queueState.current_index,
        );
      ...
      unlistenMediaControls = await listen("media:control", async (event) => {
        ...
        switch (payload.action) {
          ...
          case "seek": {
            ...
            const target =
              playerState.currentTime + direction * MEDIA_SEEK_FALLBACK_SECS;
  ...
  $effect(() => {
    const lyricsVisible =
      lyricsSidebarVisible || isFocusModeOpen || isFullScreenOpen;
  ...
  const currentQueueTrack = $derived<QueueTrack | null>(
    currentTrack
      ? {
  ...
  <div class="app" class:no-titlebar={!showTitleBar}>
        ...
        <main class="main-content">
            ...
            {:else}
              <HomeView
                ...
                onTrackShareSonglink={(track) =>
                  shareSonglinkTrack(track.id, track.isrc)}
          ...
          {:else if activeView === "album" && !selectedAlbum}
            ...
            <div class="view-error">
              ...
              <button class="view-error-back" onclick={navGoBack}
          ...
          {:else if activeView === "album" && selectedAlbum}
            <AlbumDetailView
              ...
              onArtistClick={() =>
                selectedAlbum?.artistId &&
                handleArtistClick(selectedAlbum.artistId)}
              ...
              onShowAlbumCredits={() =>
                selectedAlbum && showAlbumCredits(selectedAlbum.id)}
            />
          ...
          {:else if activeView === "library" || activeView === "library-album"}
            <LocalLibraryView
              ...
              onTrackAddToPlaylist={(trackId) =>
                openAddToPlaylist([trackId], true)}
          ...
          {:else if activeView === "playlist" && selectedPlaylistId}
            <PlaylistDetailView
              ...
              onPlaylistCountUpdate={(playlistId, qobuzCount, localCount) =>
                sidebarRef?.updatePlaylistCounts(
                  playlistId,
                  qobuzCount,
                  localCount,
                )}
          ...
          {:else if isFavoritesView(activeView)}
            ...
            {:else}
              <FavoritesView
                ...
                onPlaylistShareQobuz={sharePlaylistQobuzLinkById}
                selectedTab={getFavoritesTabFromView(activeView) ??
                  favoritesDefaultTab}
    ...
    {#if currentTrack}
      <ImmersivePlayer
        ...
        lyricsError={lyricsStatus === "error"
          ? lyricsError
          : lyricsStatus === "not_found"
            ? "No lyrics found"
            : null}
    ...
    <!-- Cast Picker -->
    <CastPicker isOpen={isCastPickerOpen} onClose={closeCastPicker} />
  </div>
{/if}
