<script lang="ts">
  ...
  interface Props {
    ...
    getTrackOfflineCacheStatus?: (
      trackId: number,
    ) => { status: OfflineCacheStatus; progress: number };
  ...
  const totalPages = $derived(
    Math.ceil(filteredArtistAlbums.length / albumsPerPage),
  );
  const visibleAlbums = $derived(
    filteredArtistAlbums.slice(
  ...
  const hasMoreThanVisible = $derived(
    filteredArtistAlbums.length > albumsPerPage,
  );
  ...
  async function loadAllRelatedAlbumDownloadStatuses() {
    if (!checkRelatedAlbumDownloaded || filteredArtistAlbums.length === 0)
      return;
    await Promise.all(
      filteredArtistAlbums.map((album) =>
        loadRelatedAlbumDownloadStatus(album.id),
      ),
    );
  }
  ...
  onMount(() => {
    ...
    if (!album?.tracks || album.tracks.length === 0) {
      console.error(
        "[DEBUG-43] AlbumDetailView mounted with no tracks - view will appear blank",
      );
    }
            ...
            {#each visibleAlbums as relatedAlbum}
              <div class="album-card-wrapper">
                <AlbumCard
                  ...
                  onPlay={onRelatedAlbumPlay
                    ? () => onRelatedAlbumPlay(relatedAlbum.id)
                    : undefined}
                  onPlayNext={onRelatedAlbumPlayNext
                    ? () => onRelatedAlbumPlayNext(relatedAlbum.id)
                    : undefined}
                  onPlayLater={onRelatedAlbumPlayLater
                    ? () => onRelatedAlbumPlayLater(relatedAlbum.id)
                    : undefined}
                  onDownload={onRelatedAlbumDownload
                    ? () => onRelatedAlbumDownload(relatedAlbum.id)
                    : undefined}
                  onShareQobuz={onRelatedAlbumShareQobuz
                    ? () => onRelatedAlbumShareQobuz(relatedAlbum.id)
                    : undefined}
                  onShareSonglink={onRelatedAlbumShareSonglink
                    ? () => onRelatedAlbumShareSonglink(relatedAlbum.id)
                    : undefined}
                  isAlbumFullyDownloaded={isRelatedAlbumDownloaded(
                    relatedAlbum.id,
                  )}
                />
              </div>
            ...
            {#if onViewArtistDiscography && filteredArtistAlbums.length >= albumsPerPage && currentPage === totalPages - 1}
              <div class="album-card-wrapper">
                <div class="view-more-card">
                  <button
                    class="view-more-cover"
                    onclick={onViewArtistDiscography}
                  >
